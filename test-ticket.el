...
(ert-deftest ticket-test-browser-transient-includes-edit-actions ()
  (let* ((close-suffix (transient-get-suffix 'ticket-browser-transient "c"))
    ...
    (should (equal (plist-get close-spec :description) "Close"))
    (should (eq (plist-get close-spec :command)
                'ticket-browser-close-ticket-at-point))
    (should (equal (plist-get dep-spec :description) "Add dependency"))
    (should (eq (plist-get dep-spec :command)
                'ticket-browser-set-dep-at-point))))
...
(ert-deftest ticket-test-browser-close-ticket-at-point-runs-close-and-refreshes ()
  (with-temp-buffer
    (ticket-browser-mode)
    (let ((inhibit-read-only t))
...
(ert-deftest ticket-test-browser-reopen-ticket-at-point-runs-reopen-and-refreshes ()
  (with-temp-buffer
    (ticket-browser-mode)
    (let ((inhibit-read-only t))
      (insert (propertize "abc\n" 'ticket-browser-id "abc")))
    ...
    (let ((tk-args nil)
      ...
      (cl-letf (((symbol-function 'ticket--run-tk-checked)
                ...
                ((symbol-function 'ticket-browser-refresh)
                 (lambda () (setq refreshed t))))
        (ticket-browser-reopen-ticket-at-point))
      (should (equal '("reopen" "abc") tk-args))
      (should refreshed))))

(ert-deftest ticket-test-browser-set-dep-at-point-selection-callback-runs-command ()
  (let ((tk-args nil)
        (refreshed nil)
        (callback nil))
...
(ert-deftest ticket-test-browser-set-parent-at-point-selection-callback-updates-frontmatter ()
  (let ((set-args nil)
        (refreshed nil)
        (callback nil))
    (cl-letf (((symbol-function 'ticket-browser-all)
      ...
      (with-temp-buffer
        ...
        (let ((inhibit-read-only t))
          (insert (propertize "abc\n" 'ticket-browser-id "abc")))
        (goto-char (point-min))
        (ticket-browser-set-parent-at-point))
      (with-current-buffer (get-buffer "*tickets*")
        (setq callback ticket-browser--selection-callback))
